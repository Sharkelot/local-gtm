import { prisma, reconcileTrustAccount, reconcileTrustAccountInputSchema } from '@local-gtm/db';
...
export async function POST(request: Request) {
  try {
    ...
    const input = reconcileTrustAccountInputSchema.parse({
      ...(await request.json()),
      idempotencyKey,
    });
    return Response.json(await reconcileTrustAccount(prisma, context, input), { status: 201 });
  } catch (error) {
