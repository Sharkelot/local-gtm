import { postLedgerEntry, postLedgerEntryInputSchema, prisma } from '@local-gtm/db';
...
export async function POST(request: Request) {
  try {
    ...
    const input = postLedgerEntryInputSchema.parse({ ...(await request.json()), idempotencyKey });
    return Response.json(await postLedgerEntry(prisma, context, input), { status: 201 });
  } catch (error) {
