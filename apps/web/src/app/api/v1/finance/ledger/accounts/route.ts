import { createLedgerAccount, createLedgerAccountInputSchema, prisma } from '@local-gtm/db';
...
export async function POST(request: Request) {
  try {
    ...
    const input = createLedgerAccountInputSchema.parse({
      ...(await request.json()),
      idempotencyKey,
    });
    return Response.json(await createLedgerAccount(prisma, context, input), { status: 201 });
  } catch (error) {
