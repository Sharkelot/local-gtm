import { createTimeEntry, createTimeEntryInputSchema, prisma } from '@local-gtm/db';
...
export async function POST(request: Request) {
  try {
    ...
    const input = createTimeEntryInputSchema.parse({ ...(await request.json()), idempotencyKey });
    return Response.json(await createTimeEntry(prisma, context, input), { status: 201 });
  } catch (error) {
