import { issueInvoice, issueInvoiceInputSchema, prisma } from '@local-gtm/db';
...
export async function POST(request: Request) {
  try {
    ...
    const input = issueInvoiceInputSchema.parse({ ...(await request.json()), idempotencyKey });
    return Response.json(await issueInvoice(prisma, context, input), { status: 201 });
  } catch (error) {
