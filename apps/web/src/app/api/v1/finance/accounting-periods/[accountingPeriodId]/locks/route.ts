...
export async function POST(
  ...
  try {
    ...
    const input = lockAccountingPeriodAccountInputSchema.parse({
      ...(await request.json()),
      accountingPeriodId,
      idempotencyKey,
    });
    return Response.json(await lockAccountingPeriodAccount(prisma, context, input), {
      status: 201,
    });
  } catch (error) {
